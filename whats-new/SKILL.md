...
name: whats-new
description: >
  Cross-reference Claude Code changelog against the current repo's skills, agents, hooks,
  and developer tooling, then categorize improvements by impact. Conversational and interactive
  — presents a visual dashboard, lets the user drill into items, select what to upgrade, and
  implements changes one-at-a-time or in batch. Repo-agnostic; discovers the stack dynamically.
  Remembers the last checkpoint so repeat runs only review new versions. Loops until the user
  says done. Use when user says "what's new", "changelog improvements", "stack audit",
  or "/whats-new".
model: opus
effort: high
---
